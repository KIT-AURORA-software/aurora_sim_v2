...
class Ackermann4WDController(Node):
    ...
    def joy_callback(self, msg):
        ...
        try:
            ...
            twist_msg.data = [linear_speed, omega]
            ...
            if abs(linear_speed) > 0.01 or abs(left_front_steering_angle) > 0.01 or abs(right_front_steering_angle) > 0.01 or abs(left_rear_steering_angle) > 0.01 or abs(right_rear_steering_angle) > 0.01:
                self.get_logger().info(
                    f'Speed: {linear_speed:.2f} m/s, omega: {omega:.2f} rad/s, Left Front Steering: {math.degrees(left_front_steering_angle):.1f}°, Right Front Steering: {math.degrees(right_front_steering_angle):.1f}°, Left Rear Steering: {math.degrees(left_rear_steering_angle):.1f}°, Right Rear Steering: {math.degrees(right_rear_steering_angle):.1f}°'
                )
